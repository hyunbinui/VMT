...
def setup(args, clear=False):
    ...
    '''
    TRAIN_VOCAB_EN, TRAIN_VOCAB_TR = args.TRAIN_VOCAB_EN, args.TRAIN_VOCAB_TR
    if clear: ## delete previous vocab
        for file in [TRAIN_VOCAB_EN, TRAIN_VOCAB_TR]:
    ...
    if not os.path.exists(TRAIN_VOCAB_TR):
        write_vocab(build_vocab(args.DATA_DIR, language='tr'), TRAIN_VOCAB_TR)
...
def main(args):
    ...
    if True:
        ...
        for epoch in range(init_epoch, args.epochs):
            ...
            logging.info('Total time used: %s Epoch %d time uesd: %s train loss: %.4f val loss: %.4f corpbleu: %.4f' % (
