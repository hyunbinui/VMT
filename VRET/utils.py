...
class Tokenizer(object):
    ...
    def split_sentence(self, sentence):
        if self.language=='en':
            ...
            return self.split_sentence_tr(sentence)
    ...
    def split_sentence_tr(self, sentence):
        ''' Break sentence into a list of characters -- Turkish '''
        toks = []
        sentence_tokenizer_object = WordTokenizer()  
        for sent in sentence_tokenizer_object.tokenize(sentence):
            toks.append(sent)
        return toks
...
def build_vocab(data_dir, language, min_count=5, start_vocab=base_vocab):
    ...
    assert language in ['en', 'tr']
    count = Counter()
    t = Tokenizer(language)
    lan2cap={'en':'en', 'tr':'tr'}

    df = pd.read_csv(data_dir+'label/'+'train.csv')

    tr = df[lan2cap['tr']].tolist()
    en = df[lan2cap['en']].tolist()

    if language == 'en':
        for d in en:
            count.update(t.split_sentence(d))
